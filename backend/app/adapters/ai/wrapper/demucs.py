...
Inherits PackageWrapper, uses the demucs package for 6-stem source separation
...
from app.adapters.ai.registry import FORMAT_PKG, MODELS_REGISTRY, SLOT_DEMUCS

logger = logging.getLogger(__name__)


class DemucsWrapper(PackageWrapper):
    """
    Demucs audio source separation wrapper (inherits PackageWrapper).

    Responsibilities:
    1. 6-stem source separation (vocals, drums, bass, guitar, piano, other)
    2. Model loading managed by the demucs package itself
    3. Device auto-switching handled by PackageWrapper
    """

    def __init__(self):
        super().__init__(slot=SLOT_DEMUCS)
        logger.info("DemucsWrapper initialized (PackageWrapper)")
