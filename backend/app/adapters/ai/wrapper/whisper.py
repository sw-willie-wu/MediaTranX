...
Inherits PackageWrapper, uses faster-whisper for speech recognition.
...
class WhisperWrapper(PackageWrapper):
    """
    Whisper speech recognition wrapper (inherits PackageWrapper).
    ...
    def __init__(self):
        ...
        logger.info("WhisperWrapper initialized (PackageWrapper)")
    ...
    def get_model_status(self, model_size: str = "medium") -> dict:
        ...
        # Check model download status directly via filesystem (no manager dependency)
        whisper_family = MODELS_REGISTRY.get(FORMAT_PKG, {}).get("whisper", {})
        slot = whisper_family.get("slot", "whisper")
        model_dir = SETTINGS.path.models / slot / model_size
        model_downloaded = (model_dir / "model.bin").exists()
    ...
    def transcribe(
        ...
        vram_needed = size_config["vram_mb"]  # noqa: F841 — used by outer mm.acquire (Wave D)

        try:
            # Load model using PackageWrapper's acquire()
            if on_progress:
