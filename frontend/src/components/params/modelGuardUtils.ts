 ...
 * 同一份比對邏輯，避免兩處漂移。
 *
 * 兩種比對形狀（req.variant 是否存在為分流準則）：
 * - family/size/quantization：variant 格式 'size:quantization'（如 '4b:Q4_K_M'）；
 *   quantization 段可空（translate 等既有路徑，行為不變）。
 * - variant/categories：variant 是純 token（如 RIFE 的 'v4.26'、Real-ESRGAN 的 'x4plus'），
 *   不做 size:quantization 拆解；categories 限定 modelStore category/subcategory 查找範圍
 *   （對照 modelStore.byCategory 的 `category === c || subcategory === c` 語意），family
 *   選配（僅在需要跨 category 收斂同家族時使用，如 enhance 的 realesrgan）。
 */
