...
function onTranslationChange(v: TranslationOptionsValue) {
  if (!v.enable_translation) {
    ...
    // SubtitleParams.vue 同款修復的檔頭註解）。buildSubmit() 已在 translate!==true 時剔除
    // 全部 translate_* 欄位（見 transcribe.meta.ts buildSubmit gate),保留不影響送出 payload；
    // modelRequirements 同理只看 translate===true，composite translate_model.get() 也是。
