/**
 * CropParams.vue 單測（統一參數元件 spec §5；批 2 Task 2.1）。
 * 覆蓋：canvasCropRect 入向寫 params、aspect 鎖定反推、外部 setParams 更新顯示、
 * one-shot 不吞合法回寫、showCropOverlay/aspectRatio UI 便利狀態 emit（沿舊 VideoCropPanel 契約）。
 */
...
describe('CropParams commit（欄位編輯 → emit）', () => {
...
describe('CropParams — canvasCropRect 入向', () => {
...
describe('CropParams — 縱橫比鎖定反推（依 committed params.width）', () => {
...
describe('CropParams — 響應式衍生＋本地編輯不打斷（one-shot pattern）', () => {
...
describe('CropParams — showCropOverlay / aspectRatio UI 便利狀態（不入 params）', () => {
...
describe('CropParams — maxW/maxH 依 fileInfo 夾限', () => {
...
describe('params/index.ts 載入表', () => {
  it('hasParamComponent("video.crop") === true', () => {
    expect(hasParamComponent('video.crop')).toBe(true)
  })

  it('METAS["video.crop"].toolKey === "video.crop"', () => {
    expect(METAS['video.crop'].toolKey).toBe('video.crop')
  })
})
