...
export const META: ToolParamMeta = {
  ...
  taskType: 'video.cut',
  // start_time default 0：CutParams 顯示層對 undefined 顯示 '00:00:00'，pipeline 節點
  // 無 seedOnFileChange——沒 default 會「看起來已填 0 但 payload 缺鍵」送後端 422
  //（e2e 實撞）；0 也正是工具頁「未動過就送 0」的既有語意。
  // end_time 維持必填無 default（無合理預設）:空值送後端 422 → execution failed;
  // v1 驗證不擋 missing(_checkField undefined skip)
  schema: [
    { name: 'start_time', type: 'number', default: 0, min: 0, step: 0.1, agentHint: 'seconds' },
