...
/**
 * image.crop 參數元件（統一參數元件 spec §5；批 4 Task 4.3）。
 * 契約：params/context/fileInfo in、update:params out——host 統一收發，本元件不呼叫 API。
 * 幾乎同構於 video/CropParams.vue（批 2 Task 2.1 範本），照抄核心 pattern，唯二差異：
 * 1. min=1（非 video 版 min=2）——PIL 裁切無 ffmpeg yuv420p 向下取偶約束。
 * 2. maxX/maxY 語意：舊 ImageCropPanel.vue:108/113 用 imageInfo.width/height - 1（非 video
 *    版的 fw - 1，數值上其實相同算法，此處沿舊 panel 逐一核對後維持一致寫法）。
 *
 ...
 * update:aspectRatio / update:showCropOverlay（沿舊 ImageCropPanel 契約，讓 ImageView 接線
 * 不變形）。canvasCropRect 是 tool context 專屬 prop，經 ToolParamHost 單根元件 attrs
 * fallthrough 透傳（host 未宣告此 prop/事件，Vue 對單根元件預設自動落到 <component :is>）；
 * pipeline context 不傳此 prop，四欄位仍照常渲染輸入框。
 */
...
// ─── showCropOverlay / aspectRatio：UI 便利狀態，不入 params（鏡射舊 panel :31-39） ──
...
// ─── 縱橫比鎖定：非 free 時 height 依「已提交」的 params.width 反推（鏡射舊 panel :59-63） ──
watch(
